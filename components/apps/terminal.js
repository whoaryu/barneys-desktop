...
export class Terminal extends Component {
    constructor(props) {
        ...
        this.child_directories = {
            root: ["books", "projects", "awesome", "suit-up", "bro-code"],
            "awesome": ["legendary-moments", "pick-up-lines", "wingman-strategies"],
            "suit-up": ["suit-collection", "color-coordinated", "epic-suits"],
            "bro-code": ["rules", "amendments", "sacred-texts"],
            books: ["The Bro Code.pdf", "The Playbook.pdf"],
            projects: ["Married to Robin", "Achieved  perfect week"]
        };
        this.state = {
            terminal: [],
        }
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className="text-yellow-400">awesomeness</div>
                        <div className="text-white mx-px font-medium">:</div>
                        <div className="text-blue-500">{this.current_directory}</div>
    ...
    childDirectories = (parent) => {
        ...
        this.child_directories[parent].forEach(file => {
            files.push(
                `<span class="font-bold mr-2 text-blue-400">'${file}'</span>`
            )
        });
        files.push(`</div>`)
        return files;
        ...
        switch (main) {
            case "cd":
                ...
                if (this.child_directories[this.curr_dir_name].includes(rest)) {
                    ...
                    result = `Navigated to ${rest}. Challenge accepted! 🎉`;
                }
                    ...
                    result = `Legendary error: ${words} not found! 😎`;
                }
                break;
            case "ls":
                let target = words[0];
                ...
                if (target in this.child_directories) {
                    ...
                    result = `Bro, cannot access '${words}': Not part of the awesome directory! 🕴️`;
                }
                break;
            case "pwd":
                let str = this.current_directory;
                result = str.replace("~", "/home/barney")
                break;
            case "suit-up":
                result = "Challenge accepted! 🕴️ Time to look awesome! 😎";
                break;
            case "legendary":
                result = "It's gonna be LEGEN - wait for it - DARY! LEGENDARY! 🌟";
                break;
            case "clear":
                this.reStartTerminal();
                return;
            ...
            case "sudo":
                ...
                break;
            default:
                result = "Challenge not accepted! Command not found. Try something awesome! 🚀";
        }
